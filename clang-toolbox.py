...
def extract_with_stream(b_url, archive_paths, dir=Path.cwd()):
	...
	with urlopen(b_url) as resp:
		...
		for entry in tf:
			local = dir / Path(entry.name).name;
			if entry.name not in targets:
				sys.stderr.write(f"{' '*80}\rSkipped: {tc} entries, totaling")
				sys.stderr.write(f" {''.join(_IEC_bformat(ts))}\r")
				tc += 1; ts += entry.size
				continue;
			else:
				sys.stderr.write(f"{' '*80}\rExtracting: {local.name}\n\r")
...
if __name__ == "__main__":
	...
	parser.add_argument("-o", "--output-dir",
		...
		script is located. Program will exit with error if PATH
		""",
	...
	if args.extraction_mode != "stream":
